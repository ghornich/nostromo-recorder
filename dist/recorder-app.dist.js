...
RecorderApp.prototype._onCapturedEvent = function (event) {
  ...
  if (command.type === 'pressKey' && this._conf.pressKeyFilter({
    event: event,
    command: command,
    recorderInstance: this
  }) === false) {
...
const RootComp = {
  view: function (vnode) {
    ...
    const toggleBtnClass = app._isRecording ? 'button--toggle-on' : 'button--toggle-off';
    return m("main", null, m("nav", null, m("button", {
      class: toggleBtnClass,
      onclick: actions.toggleRecording
    }, "Toggle recording"), m("button", {
      onclick: actions.addAssertion
    }, "Add assertion"), m("button", {
      onclick: actions.downloadOutput
    }, "Download output"), m("button", {
      class: "button--danger clear-recording-btn",
      onclick: actions.clearRecording
    }, "Clear recording")), m("div", {
      class: "content",
      onupdate: app._onContentVNodeUpdate
    }, m("section", null, m("div", {
      class: "info-bar"
    }, m("div", {
      class: "info-icon"
    }), "Press Ctrl+Shift while recording to insert an assertion.")), m("section", null, m("p", {
      class: "flex-row"
    }, "Output format:", m("select", {
      class: "output-format-dropdown",
    ...
    })))), m("section", null, m("pre", {
      class: "output"
    }, app._getFormattedOutput()))), m("a", {
      href: "#",
      id: "download-target",
      class: "hidden"
    }));
...
function renderTestfile(cmds, rawIndent) {
  const indent = rawIndent || '    ';
  const res = ['\'use strict\';', '', 'exports = module.exports = function (test) {', indent + 'test(\'\', async (t) => {'];
...
},{"../browser-puppeteer/src/commands":5}],10:[function(require,module,exports){
